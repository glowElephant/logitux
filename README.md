...
`.deb` 패키지로 설치하면 solaar 의존성과 GUI 라이브러리(PySide6)가 자동으로 설치되고
애플리케이션 메뉴에 등록됩니다.

```bash
sudo apt install ./logitux_0.1.0_all.deb
```

설치 후 메뉴에서 **logitux**를 실행하세요. (PySide6는 설치 후 pip로 자동 설치되며,
오프라인이면 `pip3 install pyside6`를 한 번 실행하면 됩니다.)
...
```bash
packaging/build-deb.sh 0.1.0   # → dist/logitux_0.1.0_all.deb
```
...
- ③-c **`.deb` 패키징** — `apt install ./logitux_*.deb` 한 줄로 의존성 자동 설치 + 메뉴 등록. `packaging/build-deb.sh`로 빌드.

계획·백엔드 레시피는 [`docs/spec.md`](docs/spec.md) 참조.
...
main.py              # 진입점
packaging/           # .deb 패키징 (③-c)
  build-deb.sh       # 빌드 스크립트 → dist/logitux_*.deb
  logitux            # /usr/bin 실행 래퍼
  postinst           # PySide6 자동 설치 + 메뉴/아이콘 캐시 갱신
  logitux.desktop    # 메뉴 등록
  logitux.svg        # 앱 아이콘
```
